...
bot.on("message", async (msg) => {
  ...
  return;
    ...
    if (text === b.buyScript) {
      ...
      if (!files.length) {
        await bot.sendMessage(chatId, "<b>📦 Script kosong</b>\n\nBelum ada file ZIP di folder scripts.", { parse_mode: "HTML" });
        return;
      }
      const rows = files.map(f => [{ text: f, callback_data: `script:${f}` }]);
...
if (data.startsWith("script:")) {
      ...
      await startPayment({ chatId, userId, productCode: "buyscript", amount: config.pricing.buyScriptDefault, meta: { filename } });
      return;
    }
  } catch (e) {
